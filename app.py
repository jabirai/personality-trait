...
def run_streamlit_app():
    ...
    model_name = st.selectbox('Select Model', ['hvp', 'svs'])
    ...
    pipeline, traits, trait_names, n_components = load_model_and_data(
        model_name)
    ...
    if user_input:
        ...
        trait_dict = dict(zip(trait_names, [round(
            float(trait), 4) for trait in scale_back(scaler, y_pred_full)[0]]))
